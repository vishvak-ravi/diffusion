...
def train(model: UNet, config: Config, writer, save: bool = True):
    ...
    for epoch in range(config.epochs):
        save_weights = epoch % config.save_weights == 0
        for images, labels in dataloader:
            start = time.time()
            images, labels = images.to(device), labels.to(device)
            writer.add_image("gt_img", images, steps, dataformats="NCHW")
            optimizer.zero_grad()
            ...
            if save_weights:
                ...
                torch.save(
                    model.state_dict(), f"weights/{config.get_str()}_{epoch}.pth"
                )
